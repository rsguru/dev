...
long long gen_srcA()
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_RMMOVQ) || (icode) == 
        ...
        (icode) == (I_RET)) ? (REG_RSP) : ((icode) == (I_LEAVE)) ? 
      (REG_RBP) : (REG_NONE));
}
...
long long gen_srcB()
{
    return (((icode) == (I_ALU) || (icode) == (I_RMMOVQ) || (icode) == 
        ...
        (icode) == (I_LEAVE)) ? (REG_RBP) : (REG_NONE));
}
...
long long gen_dstE()
{
    return ((((icode) == (I_RRMOVQ)) & (cond)) ? (rb) : ((icode) == 
        ...
        (I_RET) || (icode) == (I_LEAVE)) ? (REG_RSP) : (REG_NONE));
}
...
long long gen_aluA()
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_ALU)) ? (vala) : (
        ...
        ) ? -8 : ((icode) == (I_RET) || (icode) == (I_POPQ) || (icode) == 
        (I_LEAVE)) ? 8 : 0);
}
...
long long gen_aluB()
{
    return (((icode) == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == 
        (I_ALU) || (icode) == (I_CALL) || (icode) == (I_PUSHQ) || (icode)
         == (I_RET) || (icode) == (I_POPQ) || (icode) == (I_LEAVE)) ? 
